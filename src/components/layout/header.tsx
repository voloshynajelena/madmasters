...
export function Header({ locale }: HeaderProps) {
  ...
  return (
    <header className="main-header fixed top-0 left-0 right-0 z-50 h-20">
      ...
      {/* Drawer Menu */}
      <div
        className="drawer-list fixed right-0 top-0 h-screen box-border pointer-events-none pt-32"
        style={{
          width: isMenuOpen ? '100vw' : '100vw',
          transform: isMenuOpen ? 'translateX(0)' : 'translateX(100vw)',
          transition: 'width 475ms ease-out, transform 450ms ease, border-radius .8s .1s ease',
          borderBottomLeftRadius: isMenuOpen ? '0' : '100vw',
          backgroundColor: 'rgba(32, 32, 32, 0.95)',
          zIndex: 999,
        }}
      >
        <ul className="h-full w-full m-0 p-0 overflow-auto overflow-x-hidden pointer-events-auto">
          {navItems.map((item, index) => (
            <li
              key={item.href}
              className="list-none uppercase pointer-events-auto whitespace-nowrap"
              style={{
                transform: isMenuOpen ? 'translateX(0)' : 'translateX(100vw)',
                transition: isMenuOpen
                  ? `transform 1s ${0.08 * (index + 1)}s cubic-bezier(0.29, 1.4, 0.44, 0.96)`
                  : 'transform 0.3s ease',
              }}
            >
              <Link
                href={item.href}
                className="block text-center no-underline text-[#9e9e9e] hover:bg-[rgba(118,120,121,0.5)] transition-colors"
                style={{
                  padding: '1.2rem',
                  fontSize: 'calc(34px - .5vw)',
                }}
          ...
          {/* Social Links */}
          <li
            className="list-none pointer-events-auto"
            style={{
              transform: isMenuOpen ? 'translateX(0)' : 'translateX(100vw)',
              transition: isMenuOpen
                ? `transform 1s ${0.08 * (navItems.length + 1)}s cubic-bezier(0.29, 1.4, 0.44, 0.96)`
                : 'transform 0.3s ease',
            }}
          >
