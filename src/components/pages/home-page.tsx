...
export function HomePage({ locale, dict }: HomePageProps) {
  ...
  return (
    <FullPageScroll totalSections={5}>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center text-white px-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link
              ...
              className="px-8 py-4 bg-accent text-white text-sm tracking-wider hover:bg-accent-hover transition-colors"
            >
              {dict.cta.calculatePrice}
            </Link>
            <Link
              href={`${prefix}/work`}
              className="px-8 py-4 border border-white/40 text-white text-sm tracking-wider hover:border-white hover:bg-white/10 transition-colors"
            >
              {dict.cta.viewWork}
            </Link>
          </div>
      ...
      {/* Section 2: About */}
      <ScrollSection index={2} className="bg-[#e8e8e8]">
        <div className="flex flex-col items-center justify-center h-full px-4 py-20">
          <div className="container-section text-center">
            <span className="text-primary/50 uppercase tracking-widest text-xs">
            ...
            <div className="w-24 h-px bg-primary/30 mx-auto my-6" />
            ...
            <div className="grid md:grid-cols-2 gap-8 mt-12 text-left max-w-4xl mx-auto">
              <p className="text-primary/60 text-sm leading-relaxed">
                ...
                We define stylish and modern design. We pay special attention to page speed and
                usability using Google's recommended settings.
              </p>
              <p className="text-primary/60 text-sm leading-relaxed">
                Search engine optimization and website promotion is one of the most effective and
                inexpensive ways to attract customers and increase your profits, as well as become
              ...
              {Object.entries(dict.process).map(([key, label], index) => (
                <div key={key} className="flex items-center">
                  <div className="flex flex-col items-center px-6">
                    <div className="w-12 h-12 rounded-full border border-primary/20 flex items-center justify-center text-primary/40 mb-2">
                  ...
                  {index < 4 && (
                    <div className="hidden md:block w-12 h-px bg-primary/15" />
                  )}
                </div>
              ))}
            </div>
      ...
      {/* Section 3: Services - slides from left */}
      <ScrollSection index={3} className="bg-[#cea7bc]">
        <div className="grid grid-cols-1 md:grid-cols-2 h-full">
          {(['web', 'marketing', 'custom', 'support'] as const).map((service, i) => (
            <div
              key={service}
              className="relative group overflow-hidden flex items-center justify-center p-8"
              style={{
            ...
            >
              <div className="text-center text-white">
                ...
                <h3 className="text-xl font-bold mb-2">
                  {dict.services[service].short || dict.services[service].title.split(' ')[0]}
                  <br />
                  <span className="font-light text-lg">
                    {dict.services[service].title.split(' ').slice(1).join(' ')}
                  </span>
                </h3>
                <ul className="text-xs text-white/60 space-y-1 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                  {dict.services[service].description.split(', ').slice(0, 5).map((item, idx) => (
                    <li key={idx}>{item}</li>
                  ))}
                </ul>
              </div>
      ...
      {/* Section 4: Portfolio - revealed when section 3 slides away */}
      <ScrollSection index={4} className="bg-[#f5deb3]">
          ...
          <div className="container-section text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-w-4xl mx-auto">
              {[
                ...
                <div
                  key={index}
                  className="group relative aspect-square bg-primary/10 overflow-hidden cursor-pointer"
                >
                  <img
                    src={item.img}
                    alt={item.name}
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center">
                    ...
                    <div className="flex gap-2 my-2">
                      <button className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white text-xs hover:bg-white/30 transition-colors">
                        <BookIcon />
                      </button>
                      <button className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white text-xs hover:bg-white/30 transition-colors">
                        <LinkIcon />
            ...
            <div className="mt-8">
              <Link
                href={`${prefix}/work`}
                className="px-8 py-3 bg-primary text-white text-xs tracking-wider hover:bg-primary-light transition-colors inline-block"
              >
                {dict.cta.seeAll}
              </Link>
            </div>
      ...
      <ScrollSection index={5} className="bg-[#c0c0c0]">
        <div className="flex flex-col items-center justify-center h-full px-4 py-16">
          <div className="container-section text-center">
            ...
            <p className="text-sm text-primary/50 max-w-md mx-auto mb-8">
            ...
            <div className="flex justify-center gap-3 mt-10">
              <a
                ...
                className="w-10 h-10 rounded-full border border-primary/20 flex items-center justify-center text-primary/40 hover:text-accent hover:border-accent transition-colors"
              >
            ...
            {/* Footer bottom */}
            <div className="mt-12 pt-6 border-t border-primary/10 text-xs text-primary/40">
